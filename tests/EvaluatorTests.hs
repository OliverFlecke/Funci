...
basicTests = hspec $ do 
  describe "Evaluate basics" $ do 
    it "evaluate integer" $ evaluateString "main = 0" `shouldBe` (Number (I 0) Nothing)
    it "evaluate float" $ evaluateString "main = 0.0" `shouldBe` (Number (F 0.0) Nothing)
  ...
  describe "Basic lists" $ do 
    ...
    it "List of integers" $ do
      evaluateString "main = 1 : []" `shouldBe` (Listy (Cons (Number (I 1) Nothing) Empty))
      evaluateString "main = 1 : 2 : []" `shouldBe` (Listy (Cons (Number (I 1) Nothing) (Cons (Number (I 2) Nothing) Empty)))
      evaluateString "main = 1 : 2 : 3 : []" `shouldBe` (Listy (Cons (Number (I 1) Nothing) (Cons (Number (I 2) Nothing) (Cons (Number (I 3) Nothing) Empty))))
      -- evaluateString "main = [1,2,3]" `shouldBe` (Listy (Cons (Number (I 1) Nothing) (Cons (Number (I 2) Nothing) (Cons (Number (I 3) Nothing) Empty))))
      evaluateString "main = 1.0 : 2.0 : 3.0 : []" `shouldBe` (Listy (Cons (Number (F 1.0) Nothing) (Cons (Number (F 2.0) Nothing) (Cons (Number (F 3.0) Nothing) Empty))))
      -- evaluateString "main = [1.0,2.0,3.0]" `shouldBe` (Listy (Cons (Number (F 1.0) Nothing) (Cons (Number (F 2.0) Nothing) (Cons (Number (F 3.0) Nothing) Empty))))
      evaluateString "main = 1 + 2 : []" `shouldBe` (Listy (Cons (Number (I 3) Nothing) Empty))
...
arithmicTests = hspec $ do 
  describe "Evaluate numbering arithmics:" $ do 
    it "Addition" $ do 
      evaluateString "main = 1 + 2" `shouldBe` (Number (I 3) Nothing)
      evaluateString "main = 1.1 + 2.5" `shouldBe` (Number (F 3.6) Nothing)
    it "Subtraction" $ do 
      evaluateString "main = 2 - 1" `shouldBe` (Number (I 1) Nothing)
      evaluateString "main = 2.5 - 0.5" `shouldBe` (Number (F 2.0) Nothing)
    it "Multiplication" $ do 
      evaluateString "main = 2 * 3" `shouldBe` (Number (I 6) Nothing)
      evaluateString "main = 2.0 * 3.5" `shouldBe` (Number (F 7.0) Nothing)
    it "Divition" $ do
     evaluateString "main = 6 / 2" `shouldBe` (Number (I 3) Nothing)
     evaluateString "main = 6.0 / 2.0" `shouldBe` (Number (F 3.0) Nothing)
    it "Reminder" $ do 
      evaluateString "main = 5 % 2" `shouldBe` (Number (I 1) Nothing)
...
expressionTests = hspec $ do 
  describe "Testing let expressions -" $ do 
    it "Simple let" $ do
      evaluateString "main = let x = 1 in x" `shouldBe` (Number (I 1) Nothing)
      evaluateString "main = let y = True in y" `shouldBe` (Boolean True)
      evaluateString "main = let x = 1, y = 2 in y - x" `shouldBe` (Number (I 1) Nothing)
    it "Let with arithmics" $ do 
      evaluateString "main = let x = 1 in x + 1" `shouldBe` (Number (I 2) Nothing)
      evaluateString "main = let x = True in x || False" `shouldBe` (Boolean True)
    it "Let expressions with multiple variables" $ do 
      evaluateString "main = let x = 1, y = 2 in x + y" `shouldBe` (Number (I 3) Nothing)
      evaluateString "main = let x = True, y = False in y && x" `shouldBe` (Boolean False)
    it "Let expressions with functios" $ do 
      evaluateString "main = let f x = x in f 2" `shouldBe` (Number (I 2) Nothing)
    ...
    it "Let expressions with functions with multiple arguments" $ do
      evaluateString "main = let add x y = x + y in add 2 2" `shouldBe` (Number (I 4) Nothing)
    it "If then else expressions:" $ do 
      evaluateString "main = if True then 1 else 0" `shouldBe` (Number (I 1) Nothing)
      evaluateString "main = if False then 1 else 0" `shouldBe` (Number (I 0) Nothing)

functionTests = hspec $ do 
  describe "Testing functions without parameters -" $ do 
    it "Call function from main" $ do 
      evaluateString "main = f; f = 1" `shouldBe` (Number (I 1) Nothing)  
  ...
  describe "Testing functions with one parameter -" $ do 
    it "Calling function with a number" $ do 
      evaluateString "main = f 1; f x = x" `shouldBe` (Number (I 1) Nothing)
    ...
    it "Calling functions with expressions" $ do 
      evaluateString "main = f 1 + 2; f x = x + 1" `shouldBe` (Number (I 4) Nothing) 
  describe "Testing functions with multiple parameters -" $ do 
    it "Calling functions with multiple numbers" $ do 
      evaluateString "main = add 1 2; add x y = x + y" `shouldBe` (Number (I 3) Nothing)